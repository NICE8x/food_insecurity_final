...
read_csv("map_the_meal_gap_df.csv") %>%
  ...
  rename(
    Number_of_Food_Insecure_Individuals = "number_food_insecure_individuals",  
    Number_of_Food_Insecure_Children = "number_food_insecure_children") %>%
  pivot_longer(
    Number_of_Food_Insecure_Individuals:Number_of_Food_Insecure_Children,
    names_to = "indicator",
    values_to = "number"
  ) %>%
  mutate(county = fct_reorder(county, desc(number))) %>%
...
animate(fi_numbers, fps = 5, height = 500, width = 1000)

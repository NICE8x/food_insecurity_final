...
## *Motivation*
#### _Food Insecurity Prevalence and Significance_

Food insecurity, the state of being without reliable access to a sufficient quantity of affordable, nutritious food, is a leading health and nutrition issue in the United States. In 2017 an estimated [40 million Americans](https://www.feedingamerica.org/sites/default/files/research/map-the-meal-gap/2016/2016-map-the-meal-gap-health-implications.pdf) (13.2 percent) were food insecure.

Food insecurity is associated with numerous adverse [health outcomes](https://www.healthaffairs.org/doi/full/10.1377/hlthaff.2015.0645#:~:text=The%20majority%20of%20research%20examining,9%20and%20aggression%20and%20anxiety.), including mental health problems, diabetes, hypertension, asthma and poorer general health. As a result of the COVID-19 pandemic, millions of Americans have lost stable employment; early estimates suggest more than [50 million people](https://www.feedingamerica.org/sites/default/files/2020-10/Brief_Local%20Impact_10.2020_0.pdf), including 1 in 4 children, may be experiencing food insecurity in 2020. 

Existing [research](https://alliancetoendhunger.org/wp-content/uploads/2017/07/Hill-advocacy-fact-sheet__HUNGER-IS-A-RACIAL-EQUITY-ISSUE_Alliance-to-End-Hunger.pdf) has shown food insecurity to disproportionately impact non-White populations, as well as immigrants, those lacking health insurance, and those with lower educational attainment.  
...
Initially, our overarching question was, what does the food disparity across the United States look like on national, regional, state and county levels? After investigating the variables in our datasets, we developed more specific research questions and goals, including:
...
- Percent of Children in Food Insecure Households with Household Incomes Below 185% of the Federal Poverty Level
- Percent of Children in Food Insecure Households with Household Incomes Above 185% of the Federal Poverty Level

As these data were contained on multiple Excel files, we used the map function to import the data across files. Since column naming was not always consistent across files, the coalesce function was used to combine data from columns representing the same variables. We then generated a unique county-state field, and re-valued select county entries using stringr to be consistent with the US Healthcare.gov [Geocodes dataset](https://data.healthcare.gov/resource/geocodes-usa-with-counties.json) county naming convention.
...
#### _Food insecurity and county demographic breakdowns_
...
#### _Regional variation in food insecurity_

To view geographic trends in food insecurity indicators across the United States and over time, we also generated a Shiny Application using Plotly.  FIPS data was used to plot the data by county.  The interactive [county map](https://tessas.shinyapps.io/countymaps/) of the US allows users to view and compare food insecurity measures across all counties simultaneously. Users may select the food insecurity metric of interest, as well as the year and particular states, if desired. 
 
We also created an [interactive map](https://tessas.shinyapps.io/countymaps/) using Leaflet which contains the locations of food banks across the United States to encourage utilization and/or donation. 

To supplement the above exploratory analyses, we became interested in comparing and characterizing the prevalence of food insecurity within select New York state counties, and US states and regions. We generated [animated boxplots](https://nice8x.github.io/food_insecurity_final/dashboard_2.html) which transition through four food insecurity indicators: overall food insecurity rate, the total number of food insecure individuals, child food insecurity rate and the number of food insecure children. 
...
We also generated a region variable using the United States Census [definitions of US regions](https://www.census.gov/geographies/reference-files/2018/demo/popest/2018-fips.html) to compare region-level food insecurity indicators. The below plot compares the overall and child food insecurity rates by US region (Northeast, South, West, Midwest) over the 9 year period (2009-2017): 
...
To see population-based plots depicting the number of food insecure people and children in each selected New York State county and in each US region, visit our [NY and More dashboard](https://nice8x.github.io/food_insecurity_final/dashboard_2.html).
...
#### _Trends in national food insecurity rates_
...
map_the_meal_gap_df %>%
  ...
  kbl(caption = "<b>National Food Insecurity Metrics<b>", format = 'html') %>%
...
To volunteer or access nutrition assistance resources, visit Columbia University's [food pantry](https://thefoodpantry.studentgroups.columbia.edu/) or [Feeding America](feedingamerica.com). 
